...
在本章中做简要描述和历史回顾。
\section{混合有限元}
    混合有限元方法的专著可以参考~F.Brezzi 和~M.Fortin \cite{brezzi2012mixed}。二阶
    ...
    下面我们以~Stokes 方程为例，给出它的混合变分形式。Stokes 方程可以表示为：
    \begin{equation}
        \begin{aligned}
    ...
    压力满足~inf-sup 条件, 这就是~$P_1isoP_2P_1$ 的想法。如图~\ref{fig::p-v} 所示，因为压力
    ...
    \begin{theorem}
        ...
        不在~$\Omega$ 的边界~$\Gamma$ 上, $X_h, P_h$ 是满足~LBB 条件的, 那么问题~(\ref{eq::discreted_weak_form_stokes})
    ...
    但是需要注意到图~\ref{fig::p-v} 中，每个小三角形上的三个自由度, 其实是大的三角形~6 个自由
    度中的~3 个。因此，在做速度单元和压力单元拼装的时候，需要先找到速度单元基函数所在的大单
    ...
    \begin{figure}
      ...
      \caption{左: 压力 ~$p$ 单元, $\circ$ 为~$p$ 单元上的自由度;
               右: 4 个速度~$v$ 单元, $\bullet$ 表示~$v$ 单元上的自由度.}
      \label{fig::p-v}
    \end{figure}
...
\section{几何遗传树结构}
    ...
    在~h- 自适应中，两个耦合在一起的物理量~(如上一节提到的速度~$u$ 和压力~$p$)，有可能在不同的地方出现奇异，
...
\section{预处理方法}
    我们将分别对~Stokes 方程和~Navier-Stokes 方程，介绍预处理的方法。
    \subsection{针对~Stokes 方程}
        ...
        \end{equation}
        其中如果用满足~LBB 条件的混合元做近似，则 ~$C = 0$。因为本文中我们选取的是满足~LBB 条件的~$4P1-P1$ 元，所以我们下面均考虑 ~$C = 0$ 的情形。令 ~$\vec{v}_h \in X_h$ 和 ~$q_h \in P_h$，$X_h$ 和 ~$P_h$ 是在第一节中提到的有限元空间。我们先来定义 ~$\vec{v}_h, q_h$ 的范数：
        \begin{align}
        ...
        我们称矩阵 ~$B \vec{A}^{-1} B^T$ 为压力~Schur 补。我们考虑~$Q^{-1}B\vec{A}^{-1}B^T$ 的特征值的范围。我们先考虑边界全部是~Dirichlet 边界的流体问题，我们先给出一些定义。
        \begin{definition}
        ...
        \begin{theorem}
            \label{theo::mass_spectrual_eq} 对于 ~$\partial \Omega = \partial \Omega_D$ 的流体问题，用满足~LBB 的混合元近似并且 ~$\mathbb{R}^2$ 上的拟一致的剖分是形正则的，则
            压力~Schur 补矩阵~$B \vec{A}^{-1} B^T$ 谱等价于压力质量矩阵~$Q$:
            \begin{equation}
                \gamma^2 \leq \frac{<B \vec{A}^{-1} B^T \vec{q}, \vec{q}>}{<Q\vec{q},\vec{q}>} \leq 1 \quad \mbox{对所有的} \vec{q} \in \mathbb{R}^{n_p} \mbox{使得} \vec{q} \neq \vec{1}.
        ...
        \begin{proposition}
            \label{pro::schur_bound_neumann}对任意 ~$\int_{\partial \Omega_N} ds \neq 0$ 的流体问题，用一个一致稳定混合元近似去离散，并且这个有限元近似是建立在 ~$\mathbb{R}^d$ 上的一个形正则、拟一致的剖分上，那么压力~Schur 补矩阵满足
            \begin{equation}
        ...
        \end{equation}
        我们是在 ~$L^(\Omega)$ 空间中估计速度，在 ~$\mathcal{H}^1{\Omega}$ 中估计压力，这跟一般的~inf-sup 条件有些相反。我们先来定义速度质量矩阵 ~$\heiti{Q}$，
        \begin{equation}
        ...
        \begin{equation}
            ...
            \label{eq::coffient_stokes}
        \end{equation}
        其中~$\vec{P} \in \mathbb{R}^{n_u \times n_u}$ 和 ~$T \in \mathbb{R}^{n_p \times n_p}$ 都是对称正定矩阵。我们要到 ~$\eqref{eq::coffient_stokes}$ 的一个好的近似，意味着我们要找到合适的 ~$\vec{P}$ 近似 ~$\vec{A}$ 并且近似~Schur 补 ~$B \vec{A}^{-1} B^T$ 的矩阵 ~$T$。从定理 ~\ref{theo::mass_spectrual_eq} 我们知道，压力的质量矩阵 ~$Q$ 在合适的边界条件下是谱等价于~Schur 补矩阵的。因此我们可以选择 ~$T = Q$，并且先令 ~$\vec{P} = \vec{A}$。我们考虑广义的特征值问题
        \begin{equation}
            \left[
            \begin{array}{ll}
                A & B^T \\
        ...
        在 ~\cite{elman2005finite} 中指出，在不影响~MINRES 收敛的前提下，可以考虑用比 ~$T = \mbox{diag}(Q)$ 来作为~Schur 补矩阵的近似。并给出了 ~$T = Q$ 和 ~$T = \mbox{diag}(Q)$ 的界
        \begin{lemma}
        ...
        \begin{proof}
            ...
            对所有的 ~$\vec{v}$。
        \end{proof}
        (\cite{elman2005finite}, 第~4 章) 中指出，我们可以用一个单层多重网格循环来表示 ~$P$，虽然迭代次数
        会额外增加一些。但是用对角块直接做预处理矩阵，MINRES 的迭代步数会随着 ~$h$ 的减小而增加。
        
    \subsection{针对~Navier-Stokes 方程}
        ...
        \end{eqnarray}
        Elman 在 ~\cite{elman2005finite} 中指出，在求解 ~\eqref{eq::ossen_system} 时，用 块三角矩阵做预处理比块对角矩阵做预处理，迭代次数会减半。详见~() 我们选择用块三角矩阵
        \begin{equation}
            M = \left[
                \begin{array}{ll}
                    \vec{M}_{\vec{F}} & B^T \\
