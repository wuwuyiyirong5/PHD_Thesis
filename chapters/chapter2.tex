...
\section{预处理方法}
    ...
    \subsection{针对Stokes方程}
        ...
        \begin{theorem}
            \label{theo::mass_spectrual_eq} 对于 $\partial \Omega = \partial \Omega_D$ 的流体问题，用满足LBB的混合元近似并且 $\mathbb{R}^2$上的拟一致的剖分是形正则的，则
        ...
        \begin{proposition}
            \label{pro::schur_bound_neumann}对任意 $\int_{\partial \Omega_N} ds \neq 0$ 的流体问题，用一个一致稳定混合元近似去离散，并且这个有限元近似是建立在 $\mathbb{R}^d$ 上的一个形正则、拟一致的剖分上，那么压力Schur补矩阵满足
            \begin{equation}
        ...
        \begin{equation}
            ...
            \label{eq::coffient_stokes}
        \end{equation}
        其中$\vec{P} \in \mathbb{R}^{n_u \times n_u}$ 和 $T \in \mathbb{R}^{n_p \times n_p}$ 都是对称正定矩阵。我们要到 $\eqref{eq::coffient_stokes}$ 的一个好的近似，意味着我们要找到合适的 $\vec{P}$ 近似 $\vec{A}$ 并且近似Schur补 $B \vec{A}^{-1} B^T$ 的矩阵 $T$。从定理 \ref{theo::mass_spectrual_eq} 我们知道，压力的质量矩阵 $Q$ 在合适的边界条件下是谱等价于Schur补矩阵的。因此我们可以选择 $T = Q$，并且先令 $\vec{P} = \vec{A}$。我们考虑广义的特征值问题
        \begin{equation}
            \left[
            \begin{array}{ll}
                A & B^T \\
        ...
        因此，我们可以知道，如果用预处理的MINRES来求解的话，会很快速收敛，并且与离散的网格尺度没有关系。
     
        在 \cite{elman2005finite} 中指出，在不影响MINRES收敛的前提下，可以考虑用比 $T = \mbox{diag}(Q)$ 来作为Schur补矩阵的近似。并给出了 $T = Q$ 和 $T = \mbox{diag}(Q)$ 的界
        \begin{lemma}
        ...
        \begin{proof}
            ...
            对所有的 $\vec{v}$。
        \end{proof}
        (\cite{elman2005finite}, 第4章)中指出，我们可以用一个单层多重网格循环来表示 $P$，虽然迭代次数
        会额外增加一些。但是用对角块直接做预处理矩阵，MINRES的迭代步数会随着 $h$ 的减小而增加。
        
    \subsection{针对Navier-Stokes方程}
        用稳定的混合元近似Navier-Stokes方程，会得到与Stokes方程中类似的线性系统，我们考虑它的系数矩阵：
        \begin{equation}
            \left[
            \begin{array}{ll}
                \vec{F} & B^T \\
            ...
            \right]           
        \end{equation}
        其中 $F$ 包含对流项矩阵，不是对称的。我们可以用Krylov子空间方法中的GMRES来求解线性系统。在本文中，我们
        仅考虑Ossen系统，即
        \begin{equation}
            \left[
            \begin{array}{ll}
